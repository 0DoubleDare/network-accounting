        ...
        <div class="card-body p-4">
            <form method="GET" action="" class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <select name="point_id" class="form-select form-select-sm">
                        <option value="">Все</option>
                ...
                <div class="col-md-4">
                    ...
                    <select name="used_by" class="form-select form-select-sm">
                        <option value="">Все</option>
                ...
                <tbody class="small">
                    <?php foreach ($result['items'] as $item): ?>
                        <tr>
                            ...
                            <td>
                                <span class="text-muted"><i class="fas fa-user me-1 small"></i><?php echo htmlspecialchars($item['used_by_login'] ?? '—'); ?></span>
                            </td>
                            <td class="text-muted"><?php echo $item['used_at']; ?></td>
                            <td class="pe-4 text-truncate" style="max-width: 150px;" title="<?php echo htmlspecialchars($item['comment'] ?? ''); ?>">
            ...
            <?php if ($result['totalPages'] > 1): ?>
                <div>
                    <?php if ($result['page'] > 1): ?>
                        <a href="?action=index&page=<?php echo $result['page'] - 1; ?>&material_id=<?php echo urlencode($_GET['material_id'] ?? ''); ?>&point_id=<?php echo urlencode($_GET['point_id'] ?? ''); ?>&used_by=<?php echo urlencode($_GET['used_by'] ?? ''); ?>&date_from=<?php echo urlencode($_GET['date_from'] ?? ''); ?>&date_to=<?php echo urlencode($_GET['date_to'] ?? ''); ?>" class="text-decoration-none me-2">← Предыдущая</a>
                    <?php else: ?>
                    ...
                    <?php if ($result['page'] < $result['totalPages']): ?>
                        <a href="?action=index&page=<?php echo $result['page'] + 1; ?>&material_id=<?php echo urlencode($_GET['material_id'] ?? ''); ?>&point_id=<?php echo urlencode($_GET['point_id'] ?? ''); ?>&used_by=<?php echo urlencode($_GET['used_by'] ?? ''); ?>&date_from=<?php echo urlencode($_GET['date_from'] ?? ''); ?>&date_to=<?php echo urlencode($_GET['date_to'] ?? ''); ?>" class="text-decoration-none ms-2">Следующая →</a>
                    <?php else: ?>
